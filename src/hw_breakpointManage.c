...
static void HW_breakpointShowOne(struct HW_bpManageInfo *bpInfo, int index)
{
    ...
    printk("\tmask: \t0x%x\n", bpInfo->attr.mask);
    /*打印实际监控的地址范围*/
    printk("\trange: \t0x%llx--->0x%llx\n", bpInfo->attr.startAddr, bpInfo->attr.endAddr);
...
#ifdef KGDB_ENABLE
/*kgdb的操作函数*/
static int HW_breakpointInstallFromKgdb(unsigned long addr, int len, enum kgdb_bptype type)
{
...
static int HW_breakpointUnInstallFromKgdb(unsigned long addr, int len, enum kgdb_bptype type)
{
